...
func TestBuffer(t *testing.T) {
	...
	for _, line := range lines {
		...
		buffer, kind, value, ok, comment := fields[0], fields[1], fields[2], fields[3], fields[4]

		read := NewBuffer(buffer)
		write := NewBuffer("")
		...
		case "uint8":
			...
			actual = read.ScanUint8()
			var ex uint64
			ex, err = strconv.ParseUint(value, 10, 8)
			expected = uint8(ex)
			write.AddUint8(expected.(uint8))

		case "timestamp":

			actual = read.ScanTimestamp()
			var ex uint64
			ex, err = strconv.ParseUint(value, 10, 64)
			expected = time.Unix(int64(ex/1000), int64(1000*(ex%1000)))
			write.AddTimestamp(expected.(time.Time))

		case "ip":

			actual = read.ScanIP()
			expected = net.ParseIP(value)
			if expected == nil {
				err = errors.New("invalid ip: " + value)
			} else {
				write.AddIP(expected.(net.IP))
			}

		case "string":

			actual = read.ScanString()
			expected = strings.Replace(value, "\\x00", "\x00", -1)
			write.AddString(expected.(string))

		case "bytes":

			actual = read.ScanVarBytes()
			var ex string
			ex, err = strconv.Unquote("\"" + value + "\"")
			expected = []byte(ex)
			write.AddVarBytes(expected.([]byte))

		case "mpint":

			actual = read.ScanMPInt()
			var ex big.Int
			_, err = fmt.Sscan(value, &ex)
			expected = ex
			write.AddMPInt(expected.(big.Int))

		default:

			actual = errors.New("unimplemented")
		}

		read.ScanEof()
		if ok == "ok" {
			if read.Error != nil {
				t.Error(comment, read.Error)
			}
			...
			if !reflect.DeepEqual(actual, expected) {
				t.Error(comment, actual, "!=", expected)
			}

			if write.Error != nil {
				t.Error(comment, write.Error)
			}
			if write.String() != buffer {
				t.Error(comment, write.String(), "!=", buffer)
			}

		} else {
			if read.Error == nil {
				t.Error(comment, "did not fail", value)
			}
		}
